...
static int reb_integrator_ias15_step(struct reb_simulation* r) {
	...
	if (N3 > r->ri_ias15.allocatedN) {
		...
		r->ri_ias15.cs0 = realloc(r->ri_ias15.cs0,sizeof(double)*N3);
	...
	double* restrict const cs0 = r->ri_ias15.cs0; 
	...
	for(int k=0;k<N;k++) {
		...
		a0[3*k+2] = particles[k].az;
	 	cs0[3*k]  = r->gravity_cs[k].x;
	 	cs0[3*k+1]= r->gravity_cs[k].y;
	 	cs0[3*k+2]= r->gravity_cs[k].z;
	}
		...
		for(int n=1;n<8;n++) {							// Loop over interval using Gauss-Radau spacings
			...
			switch (n) {							// Improve b and g values
				case 1: 
					for(int k=0;k<N3;++k) {
						double tmp = g.p0[k];
						double gk = (at[k] - a0[k]) - (cs[k] - cs0[k]);
						g.p0[k]  = gk/rr[0];
						b.p0[k] += g.p0[k] - tmp;
					} break;
				case 2: 
